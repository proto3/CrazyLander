...
void arucoPosCallback(const geometry_msgs::Vector6::ConstPtr& msg)
{
    ...
    float dir_z = cons_z - msg->data.z();
    pid_z.process(dir_z, joy_msg.axes[1]);

    float dir_x_cam, dir_y_cam;
    dir_x_cam = cons_x - msg->data.x();
    dir_y_cam = cons_y - msg->data.y();
 

    if(std::abs(msg->data.x()) < 0.001)
    {
        //TODO display error
        return;
    }

    float alpha = std::atan(msg->data.y() / msg->data.x());
    float dir_x, dir_y;
...
int main(int argc, char **argv)
{
    ...
    ros::Publisher command_pub = n.advertise<sensor_msgs::Joy>("/autopilot_joy", 1);
